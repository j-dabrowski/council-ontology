...
**Level 1→2:** Analyses the completed inventory to surface the corpus typology before making Level 2 schema decisions. Shows meeting type distribution, entity averages by type and decade, all `other_content` free-text values grouped by meeting type, section heading patterns (common + rare — rare ones are potential schema gaps), and cross-reference flags. Run this after Level 1 and before writing any Level 2 schema changes.

At the end of every run, the command prints a **ready-to-paste Claude Code prompt** that summarises the schema gap signals (other_content values + rare headings) and gives explicit instructions for updating `schemas.py`, `system_prompt.txt`, and `ontology.py`. Paste it into a Claude Code session to automate the Level 2 schema update.

```bash
council typology cambridge           # prints report + writes data/cambridge_typology_review.txt
