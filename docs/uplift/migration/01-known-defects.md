...
| D-07 | One entity per real person in contested-vote ranking | `voting_power()` (`queries.py:1517-1557`) keys by `councillor_id`, labels with full given+family name | UNVERIFIED | 8 distinct real Carr councillors confirmed in DB, no duplicate `(given,family)` pairs; current query/render path wouldn't produce a bare "Carr" label — likely a retired/unregistered chart component, not read in this pass |
| D-08 | One row per real objector | `ExtractedCommunitySubmission` (`schemas.py:130-134`) — no `count` field; `system_prompt.txt:178` asks for "count and any named submitters" | PARTIAL | Join/normalisation logic (`queries.py:2905-2929`) is sound; the schema itself can't distinguish one named submitter from an aggregate-reported group |
| D-09 | Non-attendance measurable independent of vote-tally rows | `councillors_present`/`councillors_apology` **are extracted** (`schemas.py:460-461`, prompt fields `system_prompt.txt:21-22,181-182`) but never written to the DB — zero references outside `extractor.py`'s own logging | MISSING | Data is discarded at the write boundary, not absent from the pipeline; likely recoverable from `data/llm_archive/` without re-querying the API |
...
| D-14 | Displayed precision capped by what n/CI supports | Bare `round(x, 1)`/`round(x)` throughout `tests.py`, no interval to check precision against | MISSING | Confirmed by the same zero-hit statistics grep as D-10/D-11 |
...
### G-07: Contested-vote ranking display path not reproducible against current code
Target: one entity per real person in any contested-vote ranking.
Current: 8 distinct real Carr councillors confirmed in the DB with no duplicate identity rows; `voting_power()`'s current query/render path (`queries.py:1517-1557`, `PowerPanel.tsx:214`) labels with full given+family name and would not produce a bare "Carr".
Delta: the defect may have been observed against a retired/unregistered chart component (`DissentProfilesChart.tsx`/`DissentCoalitionsPanel.tsx`, unwired per `00-codebase-map.md` §5/§9) not read in this pass, or against a since-fixed councillor-identity split (one confirmed historical case exists, `tests.py:692-697`, fixed out-of-band the same day it was found).
Risk if unfixed: unknown until the actual rendering path is confirmed — carried as open.

### G-08: Objection extraction has no per-submission count field
Target: a dose-response bucket reflects true objector volume, one row per real person.
Current: `ExtractedCommunitySubmission` has no `count` field; the prompt (`system_prompt.txt:178`) asks for "count and any named submitters," permitting a one-row summary of an aggregate-reported group to look identical to a single named objector.
Delta: join/normalisation logic itself is sound and confirmed against the live DB; the schema is the limiting factor.
Risk if unfixed: any per-application objector-volume metric measures "how many objectors happened to be individually named," not "how many people actually objected," biased in an unknown direction.

### G-09: Attendance/apology data extracted but discarded before persistence
Target: non-attendance measurable independently of vote-tally rows.
Current: `councillors_present`/`councillors_apology` are extracted by the LLM on every document (`schemas.py:460-461`) but never written anywhere outside `extractor.py`'s own logging — no `meeting_attendance` table exists.
Delta: this is not a re-extraction problem — the signal is already produced by every past extraction call and is very likely recoverable from `data/llm_archive/` without re-querying the API (a hard constraint per `00-README.md`).
Risk if unfixed: the only surviving non-attendance signal (`VoteChoice.ABSENT`) conflates recusal with genuine absence at the source (`system_prompt.txt:172`), feeding directly into G-11's invalid ratio.
...
### G-14: No precision cap relative to n or CI
Target: displayed decimal precision never exceeds what the sample supports.
Current: bare `round(x, 1)`/`round(x)` throughout `tests.py`, with no interval anywhere to check precision against (same zero-hit statistics grep as G-10/G-11).
Delta: net-new capability, not a fix to an existing rule.
Risk if unfixed: false precision throughout (e.g. "100.0%" on n=1-scale cells).
...
Current: `_t_objection_dose`'s verdict uses causal framing ("numbers move outcomes") without stating the alternative explanation (non-compliant applications attract both objectors and refusals); `ConflictRecusalPanel.tsx:220-221` hardcodes the exact quoted causal phrase in frontend JSX, entirely outside `tests.py`.
Delta: the frontend instance is a second, independent claim-authoring surface the S7 invariant gate's text scan never inspects (it only scans `TestResult` fields, never component source).
Risk if unfixed: causal claims ship from a code path no existing gate covers.
...
### G-22: Corpus-wide recusal headline/grade use the blended rate, contra the frontend's own fix
Target: headline and grade lead with the must-leave-only rate everywhere, not just in per-councillor colour-coding.
Current: `_t_recusal_overall`'s headline and CRITICAL/SUPPORTIVE grade both key off the blended `stay` rate; the *per-councillor* frontend colour-coding already switched to must-leave-only (`ConflictRecusalPanel.tsx:39-44`, fixed 2026-08-11) but no corpus-wide aggregate field for must-leave-only exists on `ConflictRecusalStats` to switch the Python side to.
Delta: the same defect was found and fixed once, in one of its two occurrences; the fix didn't propagate because the two occurrences don't share a data source.
Risk if unfixed: the Scorecard, digest, and S7 gate all consume the un-fixed corpus-wide `TestResult` directly.

### G-23: Impartiality figures juxtaposed with a grade they don't affect
Target: a reader cannot mistake the 0%-impartiality-recusal figure as contributing to the panel's Critical grade.
Current: `RecusalTrendStats.impartiality_post_*` is computed correctly and correctly excluded from the grade computation, but rendered in the same panel with no visual/textual separation.
Delta: correctness of computation vs. clarity of presentation — a rendering gap, not a statistical one.
Risk if unfixed: a reasonable reader infers the figure contributed to the grade shown beside it.

### G-24: Headline and inline callout compare different era pairs
Target: every stated comparison in one panel uses the same two reference points, or explicitly labels when it doesn't.
Current: `_t_recusal_trend`'s headline compares pre→post; the confound-check sentence in the same `verdict` string compares inquiry→post — a second inconsistency independent of the financial-vs-all-must-leave denominator difference already named in D-24.
Delta: `RecusalTrendStats` has no `financial_pre_pct`/`financial_pre_n` fields — aligning the two would require adding them, not just relabelling.
Risk if unfixed: a reader who checks the panel's own internal consistency finds a self-contradiction independent of which number is "right."

### G-25: Officer-divergence panel's three compounding overstatement risks
...
Current: amendment-blindness is documented only in a `divergence.py` code comment, never in the `TestResult`; LOST and DEFERRED collapse into one `diverged` boolean; `valence=CRITICAL if near_total else SUPPORTIVE` is one fixed rule applied to every item category including planning, where frequent departure is arguably the safer signal.
