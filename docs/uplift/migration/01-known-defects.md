...
| D-31 | No unconfirmed name-match names anyone; collision count has a chance baseline | `decider_supplier_conflict()` (`queries.py`) now computes `expected_collisions_under_chance` — 2026-09-19 | FIXED | The two live/public surfaces already withheld names by construction; the null model is new: estimated from this corpus's own surname-vs-real-name overlap rate (planning-applicant names as an independent reference population), not an assumed constant. Live corpus: 2 raw collisions vs ~28 expected under chance — reported as a null (SUPPORTIVE), not a finding |
...
| D-33 | Accuracy statement with precision/recall, model version, extraction date, audited sample | `build_method_record()` gained `model_version`/`human_audit` blocks — 2026-09-19 | FIXED | Model version: which model(s) extracted the corpus, extraction date range, and a real per-document join (not a count estimate) of how many documents have a recoverable model identity vs not. Human audit: `data/audit_report.md`'s own review markers, counted — currently 0/638 filled, reported honestly as `reason: "human_review_pending"`, not a fabricated pass rate |
...
### G-31: No chance-baseline for the surname-collision count — FIXED
...
Current: `_surname_chance_baseline()` (`queries.py`) estimates each tested surname's chance-hit rate empirically against an independent, same-corpus, same-pipeline reference population (planning-applicant names — no causal path to a tender award), then sums to a Poisson-style expected collision count over the real named-award population. Wired into `DeciderSupplierConflict`, `_t_decider_supplier_conflict` (now SUPPORTIVE, not NEUTRAL, when the raw count is at or below the baseline), and `/method`'s `_build_surname_collision()` (`expected_collisions_under_chance`/`below_chance` fields, rendered in `MethodPage.tsx`). The naming-safety half was already solved on the surfaces that ship; this closes the statistical half.
Delta: this is an approximation, honestly labelled as one in the function's own docstring — it assumes tender-winner business names and planning-applicant names share a similar name-collision rate, plausible (same jurisdiction/era/pipeline) but unverified, not a rigorous linguistic model.
Risk if unfixed: n/a — closed. Live corpus result: 2 raw collisions vs ~28.04 expected under chance — now reported as a null (SUPPORTIVE), not a finding, a genuine valence change from before this fix.
...
### G-33: Accuracy statement missing model version and audit results — FIXED
...
Current: `_build_model_version()` reads `data/llm_archive/index.json` for the model(s) used, and does a real per-document join (via `_archived_pdf_paths()`, the same approach `scripts/backfill_attendance.py` uses) against `Meeting.minutes_pdf_path`/`extracted_at` to report exactly how many documents have a recoverable model identity, not a coarse count comparison. `_build_human_audit()` reads `data/audit_report.md`'s own `<!-- AUDIT: [Y/N/PARTIAL] -->` markers. Both wired into `build_method_record()` and rendered on `/method` (`ModelVersionCard`/`HumanAuditCard`, `MethodPage.tsx`). `TESTING.md`/`MAP.md`'s "five files" references updated to seven.
Delta: live corpus: 439 documents have `extracted_at` set (a separate 141 have none at all — an older gap, reported as its own field), all 439 match an archived response (`claude-haiku-4-5-20251001`). Human audit: 0 of 638 generated review items are filled in yet — reported as `reason: "human_review_pending"`, not a fabricated pass rate, since the human review genuinely hasn't happened.
Risk if unfixed: n/a — closed. The audited-sample half will start reporting a real number the moment a human fills in the first `<!-- AUDIT: -->` marker, with no further code change needed.

### G-34: Right-of-reply data has no frontend surface — rendering fixed
Target: a "sent to the council on [date]; response here" line on every named-individual claim.
Current: `ReplyStatus.tsx` renders `test.reply` — sent date, and either the verbatim response, a "declined" note, or "no response received" — wired into `AnalysisPage.tsx` right alongside `ObjectionResponse`, the shell that already renders every battery test's card. `reply` added to `ScorecardTest`/`ResolvedTest`/`resolveTests()` (previously untyped, though already present in the real JSON via `_dc()`'s full `asdict()`). Verified in a real browser (dev server + Playwright, chromium): injected a synthetic `reply` into the live placeholder JSON, confirmed the block renders correctly with no console errors, reverted the data file afterward.
Delta: was two independent gaps — a rendering gap (now closed) and a process gap (someone has to actually run `council reply-packets` and record real responses, unchanged, per this step's own scope: build against synthetic data now, don't wait on the process).
Risk if unfixed: the rendering gap is closed — the moment a real reply is recorded, it surfaces automatically, no further frontend work needed. The process gap remains exactly as before.

### G-35: Axis tick labels overlap on 14 generic-panel tests — FIXED
Target: numeric axis ticks render as visually distinct labels regardless of value range.
Current: `formatAxisTick(value, unit)` (`BatteryTestPanel.tsx`) — abbreviates >=1,000/>=1,000,000 with k/M suffixes, caps decimals to 1 for non-integers, applied via `tickFormatter` on both numeric axes (the bar chart's X-axis and the line chart's Y-axis), replacing the bare `unit` prop that just appended the unit string to whatever raw number recharts generated.
Delta: fixed at the shared component, so it applies to all 14 generic-fallback tests and any future one, not patched per-test — will hold for council #2 regardless of its value ranges.
Risk if unfixed: n/a — closed. Verified live in a browser (dev server + Playwright, chromium): `finance.eoy_spending` ($M, values up to 24,000,000) and `governance.freshman_effect` (%, two bars) both render clean, evenly-spaced tick labels.
