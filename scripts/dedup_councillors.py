...
Merge target selection (in order):
  ...
  3. Canonical with the most 2024+ votes (clear winner = ≥2× runner-up).
  4. Otherwise: normalise in place (just fix the slug/given_name), flag in report.
...
def is_real_given(given: str | None) -> bool:
    ...
    return bool(g) and g not in HONORIFICS and g not in PLACEHOLDERS
...
def find_target(
    ...
    # Multiple real names — pick the one with most 2024+ votes.
...
def run(apply: bool = False) -> None:
    ...
    for c in bad_records:
        target, reason = find_target(c, by_family, votes_2024, merging_ids)
        if target:
    ...
    # Apply
    # -------------------------------------------------------------------
    print(f"\nApplying {len(merges)} merges + {len(in_place)} in-place fixes …")

    for bad, tgt, _ in merges:
