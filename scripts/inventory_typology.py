...
def _generate_extraction_prompt(ok: list[dict], quality: dict, output_path: Path) -> str:
    ...
    out.append("  src/models/ontology.py            — new DB table per new entity type")
    out.append("  src/extraction/extractor.py       — save_extraction() for new entity types")
    out.append("  src/models/__init__.py            — export new model classes")
    out.append("")
    ...
    out.append("          Do NOT add source_quotes to nested sub-models (e.g. a councillor name")
    out.append("          nested inside an entity) — only on the top-level entity model.")
    out.append("       b. Add a list field on ExtractedMeeting for the new model.")
    ...
    out.append("     Add 'source_quotes: [string]' to each new entity in the schema block.")
    out.append("  5. Remove promoted types from the other_items item_type list in the prompt.")
    out.append("  6. For each new entity type, add a SQLAlchemy table class in ontology.py")
    out.append("     (Integer PK, ForeignKey to meetings.id, one column per schema field).")
    out.append("     Export the new class from src/models/__init__.py.")
    out.append("  7. In save_extraction() in extractor.py, add a save loop for each new entity")
    out.append("     type following the existing pattern: add the entity object, flush to get")
    out.append("     its ID, then call _ev(table_name, entity.id, entity.source_quotes) to")
    out.append("     insert provenance rows into extraction_evidence.")
    out.append("     The _ev() closure and _resolve_offset() function already exist — do not")
    out.append("     rewrite them, just call _ev() after each flush.")
